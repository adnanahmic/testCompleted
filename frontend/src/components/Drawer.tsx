...
function Content() {
	...
	const { home, todo, users } = useRoutesActive({
		home: router.home,
		todo: router.todo,
		users: router.users
	});
	...
	return (
		<div>
			...
			<Divider />
			<List>
				<ListItem button onClick={() => history.push(router.users().$)} selected={users}>
					<ListItemIcon>
						<TodoIcon todoList={todoList} />
					</ListItemIcon>
...
function TodoIcon(props: { todoList: Todo[] }) {
	let uncompletedTodos = props.todoList.filter(t => t.status === "PENDING");
